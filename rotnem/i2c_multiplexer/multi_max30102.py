...
class MAX30102_1():
    ...
    # by default, this assumes that the device is at 0x57 on I2C Multiplexer channel 0
    def __init__(self, channel=0, address=0x57, gpio_pin=7):
        print("Channel: {0}, address: 0x{1:x}".format(channel, address))
        self.address = address
        #self.channel = channel
        #self.bus = smbus.SMBus(self.channel)
        self.multi_channel = channel #I2C multiplexer no. 0
        self.i2c_multi = I2C_Multiplexer.i2c_multiplexer(I2CMultiAddr)
        ...
        # reg_data = self.bus.read_i2c_block_data(self.address, REG_INTR_STATUS_1, 1)  # org code
        reg_data = self.i2c_multi.readfrom_mem(channel,address,REG_INTR_STATUS_1, 1)
    ...
    def shutdown(self):
        ...
        self.i2c_multi.writeto_mem(self.multi_channel,self.address,REG_MODE_CONFIG, [0x80])
    ...
    def reset(self):
        ...
        self.i2c_multi.writeto_mem(self.multi_channel,self.address,REG_MODE_CONFIG, [0x40])
    ...
    def setup(self, led_mode=0x03):
        ...
        self.i2c_multi.writeto_mem(self.multi_channel,self.address,REG_INTR_ENABLE_1, [0xc0])
        self.i2c_multi.writeto_mem(self.multi_channel,self.address,REG_INTR_ENABLE_2, [0x00])

        # FIFO_WR_PTR[4:0]
        #self.bus.write_i2c_block_data(self.address, REG_FIFO_WR_PTR, [0x00])
        self.i2c_multi.writeto_mem(self.multi_channel,self.address,REG_FIFO_WR_PTR, [0x00])

        # OVF_COUNTER[4:0]
        #self.bus.write_i2c_block_data(self.address, REG_OVF_COUNTER, [0x00])
        self.i2c_multi.writeto_mem(self.multi_channel,self.address,REG_OVF_COUNTER, [0x00])
        

        # FIFO_RD_PTR[4:0]
        #self.bus.write_i2c_block_data(self.address, REG_FIFO_RD_PTR, [0x00])
        self.i2c_multi.writeto_mem(self.multi_channel,self.address,REG_FIFO_RD_PTR, [0x00])


        # 0b 0100 1111
        # sample avg = 4, fifo rollover = false, fifo almost full = 17
        #self.bus.write_i2c_block_data(self.address, REG_FIFO_CONFIG, [0x4f])
        self.i2c_multi.writeto_mem(self.multi_channel,self.address,REG_FIFO_CONFIG, [0x4f])
        ...
        self.i2c_multi.writeto_mem(self.multi_channel,self.address,REG_MODE_CONFIG, [led_mode])
        ...
        self.i2c_multi.writeto_mem(self.multi_channel,self.address,REG_SPO2_CONFIG, [0x27])

        # choose value for ~7mA for LED1
        #self.bus.write_i2c_block_data(self.address, REG_LED1_PA, [0x24])
        self.i2c_multi.writeto_mem(self.multi_channel,self.address,REG_LED1_PA, [0x24])

        # choose value for ~7mA for LED2
        #self.bus.write_i2c_block_data(self.address, REG_LED2_PA, [0x24])
        self.i2c_multi.writeto_mem(self.multi_channel,self.address,REG_LED2_PA, [0x24])

        # choose value fro ~25mA for Pilot LED
        #self.bus.write_i2c_block_data(self.address, REG_PILOT_PA, [0x7f])
        self.i2c_multi.writeto_mem(self.multi_channel,self.address,REG_PILOT_PA, [0x7f])
    ...
    def set_config(self, reg, value):
        ...
        self.i2c_multi.writeto_mem(self.multi_channel,self.address,reg,value)
    ...
    def read_fifo(self):
        ...
        reg_INTR1 = self.i2c_multi.readfrom_mem(self.multi_channel,self.address,REG_INTR_STATUS_1, 1)
        reg_INTR2 = self.i2c_multi.readfrom_mem(self.multi_channel,self.address,REG_INTR_STATUS_2, 1)

        # read 6-byte data from the device
        #d = self.bus.read_i2c_block_data(self.address, REG_FIFO_DATA, 6)
        d = self.i2c_multi.readfrom_mem(self.multi_channel,self.address,REG_FIFO_DATA, 6)
...
class MAX30102_2():
    # by default, this assumes that physical pin 11 (GPIO 17) is used as interrupt
    # by default, this assumes that the device is at 0x57 on I2C Multiplexer channel 2
    def __init__(self, channel=2, address=0x57, gpio_pin=11):
        print("Channel: {0}, address: 0x{1:x}".format(channel, address))
        self.address = address
        #self.channel = channel
        #self.bus = smbus.SMBus(self.channel)
...
class MAX30102_3():
    # by default, this assumes that physical pin 13 (GPIO 27) is used as interrupt
    # by default, this assumes that the device is at 0x57 on I2C Multiplexer channel 4
    def __init__(self, channel=4, address=0x57, gpio_pin=13):
        print("Channel: {0}, address: 0x{1:x}".format(channel, address))
        self.address = address
        #self.channel = channel
        #self.bus = smbus.SMBus(self.channel)
